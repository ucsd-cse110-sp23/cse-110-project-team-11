...
public class Update {
    ...
    public static void main(String[] args) {
        String uri = "mongodb+srv://Cluster95779:YV50WnpyZEp9@cluster95779.lfr31so.mongodb.net/?retryWrites=true&w=majority";
        JsonWriterSettings prettyPrint = JsonWriterSettings.builder().indent(true).build();


        try (MongoClient mongoClient = MongoClients.create(uri)) {
            MongoDatabase sampleTrainingDB = mongoClient.getDatabase("cse110");
            MongoCollection<Document> gradesCollection = sampleTrainingDB.getCollection("user_information");

            JSONObject sample = new JSONObject();
            JSONArray sampleArray = new JSONArray();
            JSONObject sampleObject = new JSONObject();
            sampleObject.put("question", "What is your name");
            //JSONObject sampleObject2 = new JSONObject();
            sampleObject.put("answer", "John");
            sampleArray.put(sampleObject);
            //sampleArray.put(sampleObject2);
            sample.put("historyPrompt", sampleArray);


            // update one document
            Bson filter = eq("email", "1");
            Bson updateOperation = set("history_prompt", Document.parse(sample.toString()));
